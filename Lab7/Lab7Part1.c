...
void comp(char board[][26], int num, char colour);
...
void comp(char board[][26], int num, char colour)
{
    char rowletter, colletter, a = 'a', b = 'a', opcolour, s='a', t='a';
    int i, j, deltarow, deltacol, score=0, cnt, x, y, p, q, inew, jnew;
    if (colour == 'B')
        opcolour = 'W';
    else if (colour == 'W')
        opcolour = 'B';
    ...
    if(board[x][y] == 'U')
    {
        for (deltarow = -1; deltarow < 2; deltarow++)
            for (deltacol = -1; deltacol < 2; deltacol++)
            {
                if(!(deltarow == 0 && deltacol == 0) && checklegalindirection(board, num, s, t, colour, deltarow, deltacol) && positioninbounds(num, s + deltarow, t + deltacol))
                {
                    x=inew;
                    y=jnew;
                    board[x][y] = colour;
...
void winner(char board[][26], int num, char colour)
{
    int i, j, colscore=0, opcolscore=0;
    char opcolour;
    if(colour == 'B')
        opcolour = 'W';
    else if (colour == 'W')
